...
const resultPlayAgainButton = document.querySelector(".result-play-again-button");
...
const resetGame = () => {
  ...
  for(let i = 0; i < userRanking.length; i++) {
    ...
    if(element) {rankingList.removeChild(element)};
  };
...
const gameRanking = () => {
  ...
  for(let i = 0; i < userRanking.length; i++) {
    ...
    userElement.textContent = `${i + 1}º- ${userRanking[i].userName}: ${userRanking[i].points} puntos.` 
